<?php
	function db_connect(){
		$conn = mysqli_connect("localhost", "mad4saoi_booksmart", "XO,5%DH*IDm9", "mad4saoi_booksmart");
		if(!$conn){
	...
	function setCustomerId($id,$name, $address, $city, $zip_code, $country){
		$conn = db_connect();
	...
	function searchBooks($conn,$query, $pubid,$isbn,$book_title, $book_author){
		$query = "SELECT * from books  WHERE publisherid = '$query',";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
